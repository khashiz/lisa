...
if ($this->ticket->status_id == RST_STATUS_CLOSED)
{
	?>
        ...
        <div class="uk-alert uk-alert-danger uk-padding-small uk-text-center uk-border-pill uk-margin-remove"><p class="font f500 uk-text-small"><?php echo JText::_('RST_TICKET_REPLIES_CLOSED'); ?></p></div>
	<?php
...
else
{
	if ($this->canReply)
	{
		...
		{
			?>
			<div><button type="button" class="uk-button uk-button-success uk-button-large uk-border-pill uk-box-shadow-small" id="com-rsticketspro-reply-button" onclick="RSTicketsPro.showReply(this);"><?php echo JText::_('RST_TICKET_REPLY'); ?></button></div>
			<?php
		}
		?>
		<div id="com-rsticketspro-reply-box" <?php if (!$showReply) { ?>class="hidden"<?php } ?>>
			<h3 class="font f700 uk-text-primary uk-h5"><?php echo JText::_('RST_REPLY_TO_TICKET'); ?></h3>
			<?php
			...
			?>
			<div class="uk-margin-top"><button type="button" onclick="Joomla.submitbutton('ticket.reply')" class="uk-button uk-button-success uk-border-pill uk-box-shadow-small"><?php echo JText::_('RST_TICKET_SUBMIT'); ?></button></div>
        </div>
		<?php
	}
}