        ...
        <div class="uk-child-width-1-2 uk-child-width-auto@s uk-grid-small" id="ticket-buttons" data-uk-grid>
            ...
            if (!$this->isPrint)
            {
                ...
                if ($this->ticket->status_id == RST_STATUS_CLOSED && $this->canOpenTicket)
                {
                    ?>
                        <div>
                            <a href="<?php echo JRoute::_('index.php?option=com_rsticketspro&task=ticket.reopen&id='.$this->ticket->id); ?>" class="uk-button uk-button-success uk-border-pill uk-box-shadow-small uk-width-1-1"><i class="fas fa-refresh"></i><?php echo JText::_('RST_TICKET_OPEN'); ?></a>
                        </div>
                    <?php
                }
                elseif ($this->ticket->status_id != RST_STATUS_CLOSED && $this->canCloseTicket)
                {
                    ?>
                        <div>
                            <a href="<?php echo JRoute::_('index.php?option=com_rsticketspro&task=ticket.close&id='.$this->ticket->id); ?>" class="uk-button uk-button-danger uk-border-pill uk-box-shadow-small uk-width-1-1"><i class="fas fa-close"></i><?php echo JText::_('RST_TICKET_CLOSE'); ?></a>
                        </div>
                    <?php
                }
            }
                ...
                <?php if (is_null($user)) { ?>
                    <div>
                        <div class="uk-alert uk-alert-primary uk-padding-small uk-border-rounded">
                            <p class="uk-text-center uk-text-tiny font f500"><?php echo '<span class="f700">'.$this->showDate($message->date).'</span> ، '.strip_tags(RSTicketsProHelper::showMessage($message),'<span>'); ?></p>
                        </div>
                        ...
                        <div class="uk-grid-small" data-uk-grid>
                            ...
                            <div class="uk-width-2-3">
                                <div class="uk-background-muted uk-border-rounded uk-padding-small">
                                    <div class="font uk-text-tiny uk-margin-small-bottom f300 uk-text-muted"><?php echo JText::sprintf('TICKET_INFO', '<span class="uk-display-inline-block f500">'.$text.'</span>', '<span class="uk-display-inline-block f500">'.$this->showDate($message->date).'</span>') ?></div>
                                    <div class="uk-text-justify font f500 uk-text-dark"><?php echo RSTicketsProHelper::showMessage($message); ?></div>
                                </div>
                            </div>
